...
func runTOMLBenchmarks(corpusDir: URL, sizes: [String]) {
    ...
    for size in sizes {
        ...
        guard let data = try? Data(contentsOf: url),
            ...
            let text = renderTOML(RawValue(value), sections: true)
        else { continue }
    ...
    func table(
        ...
        for doc in docs {
            ...
            ratios.append(ratio)
            print(
                pad(doc.size, 10, right: true) + pad("\(doc.bytes.count)", 10)
