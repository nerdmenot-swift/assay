...
| `ColumnarSource` + `_assayBatch` | **built** — the surviving half. Field manifest, `BoundPlan` two-phase binding, Arrow-style validity masks. **8.4× the tree path at a flat ~10 ns/row** (2026-09-10; it was 1.27× at 53 ns until a per-row diagnostic-path allocation was removed — a fix that had been made on 09-06 and wrongly reverted on 09-07 as "byte-identical machine code"; the row index now reaches issues through `IssueSink._enterRow`, two integers, and `sink.add` inserts it cold); 1.13× called generically from another module. The `colfloor` arm guards it against a hand-written floor |
| `ColumnDecodable` + `BytesColumn` | **built and measured 2026-08-30** — the columnar extension point, so a consumer's own scalar (`Timestamp`, `Decimal128`) crosses a column store without Assay learning its name. Carrier is an `associatedtype`, not a field attribute, because a macro is syntactic and cannot see conformances. **NOT free, corrected 2026-09-11: ~3 ns/row, 1.66–2.02× a built-in `Int64` column.** It was recorded as free (0.97×, three runs) and it never was — both sides measured ~42 ns/row until the per-row allocation went on 2026-09-10, and a 3 ns difference inside 42 ns reads as noise. The re-measurement taken that day kept the conclusion and does not reproduce; the first commit after the fix measures 1.80×. The cost is structural and in the expansion: a built-in column's validity mask is hoisted before the row loop, a `ColumnDecodable` one projects `.nulls` AND `.metadata` out of the column struct per row. Hoisting them is the obvious fix and is NOT done (codegen change: goldens, tests, compile-time gate). The design claim survives: the generic call is still once per COLUMN and the per-row call concrete — the shape `KeyedSource` got wrong at 1.6–4.7×. `ColumnMetadata` carries the unit as data. Bytes are flat-plus-offsets: materialising costs 1.53×. `docs/COLUMN-DECODABLE.md` |
| `Date` + `UUID` columnar, in `AssayFoundation` | **built 2026-08-30** — `Date` carries `ColumnBuffer<Int64>` with the unit from `ColumnMetadata` (divide before converting: 1.7e18 ns needs 61 bits against Double's 53). `UUID` carries `BytesColumn`, taking 16 raw bytes or 36 canonical text bytes, and **also gained the tree path it never had** — `var id: UUID` was not a legal field before. Acceptance is `FormatValidators.isUUID`'s exactly, on every path. 8,079 differential checks vs Foundation in `DiffFuzz` (the test target cannot import Foundation: `_Testing_Foundation` has a macOS 13 floor) |
...
| Rows: `RowBatch`, `RowDecoder<T>`, text cells, `RowSink` | **built 2026-09-11** — the generic row-shaped path, `docs/ROWS.md`. The transpose a SQL driver or CSV reader would otherwise write, in AssayCore, conforming to `ColumnarSource`; per-cell calls go INTO a concrete struct (the direction `KeyedSource` got wrong). **~40 ns/row on the 8-column row against ~70 for a RawValue per row; a hand transpose is 28; the gate of 1.2× was missed at 1.45× and shipped with the record** — seven storage shapes profiled, the killers were dynamic exclusivity on class-boxed columns (CLAUDE.md rule 3), a borrowed `String` parameter (retain/release per cell — `consuming` fixed it), and a per-cell mask RMW through `self` (serialised on store forwarding). Text cells under `coerceScalars` by the tree path's rules, one definition — but reaching them through `RowBatch` needs **`inferColumnKinds: true`**, added 2026-09-11, because kinds otherwise come from the manifest and every text cell offered to a numeric field was rejected. CSV-through-`RowDecoder` was the use case the feature was built for, was advertised in three documents, and did not work; every test of it used a hand-written `ColumnarSource` serving text. **40.9 ns/row inferred against 32.7 declared, both rows of one run** — two ways to make it free were built and measured (a branch in `append(string:)`, and inferring for everyone) and both cost the declared path, so it is a flag. `@Check` and `@Transform` run on the batch path now (neither did); `@AsyncCheck` + `sources:` refused. `encodeRow(into: RowSink)` is the write side, 1.3 ns/row vs 59 for the tree. `sources` compile-time arm: 164 ms/type at the widest, the reason it is opt-in |
| `parse(toml:)` | **built 2026-09-10** — `AssayTOML`, a hand-written TOML 1.0.0 parser on the `RawValue` path. **710/710 on the official toml-test suite** (210 valid to the exact value, 501 invalid refused) on the first run, in CI; toml++ (TOMLKit) as the differential oracle. The whole difficulty is the redefinition rules, implemented as "every value except an open table is closed when parsed" plus one origin tag per table. Date-times project to RFC 3339 strings so `Date` fields decode unchanged. Encoding omits nil members and reports every other null (TOML has no null). **1.09× toml++ on the tree, 1.81× TOMLKit's Codable decoder** — a tree decoder, the JSON thesis does not apply. `docs/TOML.md` |
