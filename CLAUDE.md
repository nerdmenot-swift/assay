...
| `@Schema(discriminator:)` — TAGGED unions | **built 2026-09-09** — decode only, JSON only. Scan keys for the tag (values skipped structurally), rewind, decode the branch over the whole object. **ROADMAP's "both primitives already exist" was one short**: `seek(to:)` restores the cursor and `rollback(to:)` the issues, but neither restores container DEPTH — an ordinary failure is balanced, a MALFORMED CONTAINER is not, measured both ways. `AssayReader.Mark` / `restore(_:)` is the fix. Tagged first because three of `UNIONS.md`'s four hard questions do not apply to it. Untagged, encoding and the RawValue path are all REFUSED at expansion rather than silently omitted |
