...
| `ColumnDecodable` + `BytesColumn` | **built and measured 2026-08-30** — the columnar extension point, so a consumer's own scalar (`Timestamp`, `Decimal128`) crosses a column store without Assay learning its name. Carrier is an `associatedtype`, not a field attribute, because a macro is syntactic and cannot see conformances. **Free: 4.06 vs 4.16 ns/row against a built-in `Int64` column, 0.97×** (42.29 vs 42.65 until 2026-09-10 — same ratio, same per-row allocation on both sides), because the generic call is once per COLUMN and the per-row call is concrete — which is exactly the shape `KeyedSource` got wrong. `ColumnMetadata` carries the unit as data. Bytes are flat-plus-offsets: materialising costs 1.53×. `docs/COLUMN-DECODABLE.md` |
