...
| **A document key written with a JSON escape never matches its field** | **Open, found 2026-09-19** by an encode test for keys containing a quote. The JSON decoder matches keys on their RAW bytes: `scanKey` computes `KeyRange.simple` (no backslash in the key) and nothing reads it, so `{"a\/b": 1}` does not match `@Key("a/b")` although RFC 8259 says they are the same key. The case that matters is interop, not odd keys: Python's `json.dumps` escapes every non-ASCII character by default (`ensure_ascii=True`), so a field named `café` arrives as `"caf\u00e9"` and is reported MISSING. The fix belongs in dispatch, on the cold side of `simple` (unescape, then match), and must cost the unescaped path nothing; `count.py` decides that. Pinned by `EncodingTests.escapedDocumentKeys` under `withKnownIssue`. |
