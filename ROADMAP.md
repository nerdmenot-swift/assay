...
| **YAML indentless block sequences are not parsed — and one form is MIS-parsed** | **Open, found 2026-09-19** by the portable scaling test, whose fixture used the form. `items:` followed by `- a` at the KEY's indentation is valid YAML 1.2 and is how Kubernetes manifests, GitHub Actions workflows and docker-compose files are commonly written. `items:\n- a\n- b` is refused (`expected ':' after mapping key`); `items:\n- name: x\n  n: 0` is refused; **`items:\n- name: x` is accepted as `{items: "", "- name": "x"}`**, so a schema then reports `items` as the wrong type rather than the document as malformed. Indenting the sequence under the key works. The Yams differential never caught it because the corpus never uses the form. Pinned by `YAMLIndentlessSequenceTests` under `withKnownIssue`, which fails the moment it is fixed. Not fixed in the efficiency work that found it, deliberately: it is a parser change with its own differential run. |
