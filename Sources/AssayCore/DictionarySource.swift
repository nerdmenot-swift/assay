...
public struct DictionarySource: KeyedSource, ~Copyable {
    ...
    @inlinable
    public borrowing func has(_ key: StaticString) -> Bool {
        values[String(describing: key)] != nil
    }

    @inlinable
    public borrowing func isNull(_ key: StaticString) -> Bool {
        if case .null = values[String(describing: key)] { return true }
        return false
    }

    @inlinable
    public borrowing func int64(_ key: StaticString) -> Int64? {
        values[String(describing: key)]?.int
    }

    @inlinable
    public borrowing func double(_ key: StaticString) -> Double? {
        values[String(describing: key)]?.double
    }

    @inlinable
    public borrowing func bool(_ key: StaticString) -> Bool? {
