...
//
// `Article.parse(toml:)` on a schema that does not declare TOML used to say
//
//     referencing static method 'parse(toml:limits:sourceName:)' on 'RawDecodable'
...
// which every schema conforms to, and are `unavailable`. A type that DOES decode TOML
