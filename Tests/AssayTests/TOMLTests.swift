...
struct TOMLValueTests {
    ...
    func strings() throws {
        ...
        emoji = "\U0001F600"
        """#)
        #expect(doc["basic"]?.string == "I'm a string. \"You can quote me\". Name\tJosé\nLocation\tSF.")
        ...
        #expect(doc["quotes"]?.string == "Here are fifteen quotation marks: \"\"\"\"\"\"\"\"\"\"\"\"\"\"\".")
        #expect(doc["adjacent"]?.string == "\"This,\" she said, \"is just a pointless statement.\"")
        #expect(doc["mllit"]?.string == "The first newline is\ntrimmed in raw strings.\n   All other whitespace\n   is preserved.\n")
    ...
    func tables() throws {
        ...
        #expect(fruits[0]["varieties"]?.array?.compactMap { $0["name"]?.string } == ["red delicious", "granny smith"])
...
struct TOMLRefusalTests {
    ...
    @Test("every specification-stated invalid document is refused, with its own code", arguments: [
        // keys
        ...
        ("[[fruits]]\nname = \"apple\"\n[[fruits.varieties]]\nname = \"x\"\n[fruits.varieties]", "toml_redefined_table"),
        ("[[fruits]]\nname = \"apple\"\n[fruits.physical]\ncolor = \"red\"\n[[fruits.physical]]", "toml_redefined_table"),
    ...
    func limits() {
        ...
        #expect(sink.issues.first?.code == .depthExceeded)
        sink = IssueSink(limits: limits)
        _ = TOML.decode(Array("a = {b = {c = {d = {e = {f = 1}}}}}".utf8), into: &sink, limits: limits)
        #expect(sink.issues.first?.code == .depthExceeded)
...
struct TOMLSchemaTests {
    ...
    func decodes() throws {
        ...
        let j = try TOMLConfig.parse(json: #"""
    ...
    func sameErrors() {
        let d = TOMLConfig.diagnose(toml: """
        title = "svc"
        port = 70000
        ratio = "high"
        enabled = true
    ...
    func dates() throws {
        let d = try TOMLDated.parse(toml: """
    ...
    func widths() throws {
        let ok = try TOMLWide.parse(toml: "u8 = 255\ni16 = -32768\nu64 = 9223372036854775807\nf = 1e3")
    ...
    func encodeNull() {
        var sink = IssueSink()
        _ = TOML.encode(.mapping([.init(key: "a", value: .sequence([.int(1), .null]))]), into: &sink)
    ...
    func negotiation() throws {
        let body = Array("ip = \"1\"\nrole = \"r\"\n".utf8)
        let s = try TOMLServer.parse(body: body, contentType: "application/toml", accepting: [.toml])
        #expect(s.ip == "1")
        let t = try TOMLServer.parse(body: body, contentType: "application/vnd.x+toml; charset=utf-8", accepting: [.toml])
        #expect(t.role == "r")
        let refused = TOMLServer.diagnose(body: body, contentType: "application/toml", accepting: [.json])
