...
| 10 | The matrix was JSON-only, so nothing counted the YAML and TOML parsers. New `yaml` and `toml` tasks (fixtures rendered into block YAML and `[[items]]` TOML by the library's own writers, outside the timed region) found: YAML 8,014 blocks per `base` call, the same doubling as JSON.Value; TOML **58,022 blocks, 29 per record**, 2.4× YAML's instructions: array growth, one allocation per string in `scanBasicString` (10,000), three `Dictionary`s per record (6,001), and `TOML.Node` is an `indirect enum`, a box per node | YAML: shape memory, shared from AssayCore as `_ShapeHints`. TOML: next | **YAML KEPT 2026-09-19**: base/yaml −10.9% instructions, 8,014 → 2,018 blocks; array-10/yaml −12%, 18,014 → 4,022; the Yams/libyaml differential still agrees on every corpus document. TOML open | YAML kept; TOML open |
