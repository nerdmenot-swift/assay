...
// ---------------------------------------------------------------------------
//
// Generated from CHANGELOG.md rather than written twice. A docs site that does not say what
// changed makes the reader diff two tags to find out, and a hand-copied changelog is the
// first page to go stale — this one cannot, because there is only one copy of the words.
//
// Two transformations, both mechanical: drop the file's own `#` title (the page has one
// from its frontmatter), and rewrite repository-relative links to GitHub, because
// `[ROADMAP.md](ROADMAP.md)` resolves to nothing on a docs site.
const REPO_URL = 'https://github.com/nerdmenot-swift/assay/blob/main/'
const changelog = readFileSync(join(REPO, 'CHANGELOG.md'), 'utf8')
  .replace(/^# Changelog\n+/, '')
  .replace(/\]\((?!https?:|\/|#)([^)]+)\)/g, `](${REPO_URL}$1)`)

writeFileSync(
  join(HERE, '..', 'src', 'content', 'docs', 'reference', 'changelog.md'),
  `---
title: Changelog
