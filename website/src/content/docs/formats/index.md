...
- **It is fast.** A survey of the Swift YAML options found one allocating a class per node,
  ...
  probe. Assay's YAML parser measures 6.6× it.
...
Two consequences. The tree path is slower — it builds a tree first. And your rules, keys,
presence states and checks are identical across formats, because they all run on the far
side of the projection. That is the trade, and it is the right way round: JSON is the
format people decode in a hot loop.
